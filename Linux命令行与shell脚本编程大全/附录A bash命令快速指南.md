...
```
内建命令已经内建在shell中，在使用这些命令时速度就会快很多。
...
```
除了内建命令外，bash shell还使用外部命令来让你操控文件系统以及处理文件和目录。
...
```
虽然环境变量不是命令，但它们通常会影响shell命令的执行，所以了解这些shell环境变量很重要。

* 										含有所有命令行参数(以单个文本值的形式)
@ 										含有所有命令行参数(以多个文本值的形式)
# 										命令行参数数目 12 ? 最近使用的前台进程的退出状态码
- 										当前命令行选项标记
$ 										当前shell的进程ID(PID) 13 ! 最近执行的后台进程的PID
0 										命令行中使用的命令名称
_ 										shell的绝对路径名
...
TMOUT									select和read命令在没输入的情况下等待多久(以秒为单位)。默认值为零，表示 无限长
